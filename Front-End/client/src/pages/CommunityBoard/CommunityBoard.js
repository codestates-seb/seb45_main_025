...
function CommunityBoard() {
  ...
  const editComment = async (commentId, updatedContent) => {
    ...
    try {
      ...
      const response = await axios.patch(
        `${URI}/board/${id}/comment/${commentId}`,
        {
      ...
      const updatedComments = comments.map((c) =>
        c.commentId === commentId ? response.data : c
      );
      setComments(updatedComments);

      // 해당 댓글의 편집 상태를 false로 설정하여 편집 모드를 종료합니다.
      const updatedEditStates = editStates.map((state, index) =>
        index === commentId ? false : state
      );
  ...
  const deleteComment = async (commentId) => {
    ...
    try {
      ...
      await axios.delete(`${URI}/board/${id}/comment/${commentId}`, {
        headers: { Authorization: access_token }
      });
      // 댓글 목록에서 삭제된 댓글 제거
  ...
  return (
    <>
      ...
      <CommunityBoardContainer>
        <PostBox>
          <PostTitleBox>
            <PostTitle>{boardData.title}</PostTitle>
            {isMenuVisible && (
              <MenuIcon1
