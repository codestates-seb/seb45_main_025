...
function CommunityBoard({ title, content, boardId, profilePicture, postDate, viewCount, username }) {
  ...
  const fetchComments = async () => {
    try {
      const response = await axios.get(`${URI}board/${boardId}/comment`);
  ...
  const editComment = async (commentId, updatedContent) => {
    try {
      ...
      const response = await axios.patch(`${URI}/board/${boardId}/comment/${commentId}`, {
        "content": content
      });
  ...
  const deleteComment = async (commentId) => {
    try {
      ...
      await axios.delete(`${URI}/board/${boardId}/comment/${commentId}`);
      // 댓글 목록에서 삭제된 댓글 제거
